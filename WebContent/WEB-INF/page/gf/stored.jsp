...
<div id="wrapper">
   <%--  <jsp:include page="./com/left.jsp" /> --%>
    <div id="page-wrapper" class="th">
       	<table id="showitems">
       	  <strong id="cc">商品信息:</strong>
       	  <input type="text" name="name"/><button>搜索</button>	
       	  <tbody>
			  <tr id="show">
	                          ...
	                          //append是内部追加  html是内部替换  attr是仅用于设置属性
	                              .append($("<tr></tr>").attr("named", "hehe"+index)
	                                 /*  .append($("<td></td>").html("<input type='checkbox' class='cid' name='cid' value='" + item.id + "'>")) */
	                                  ...
	                                  .append($("<td></td>").attr("width", "140").html("<button value='"+item.id+"' temp='1' onclick='updateStat(this)'>下架</button> &emsp;|&emsp;<a href='${root}/admin/data/showItems?id="+item.id+"'>查看</a>")) //"+cart.id+"  
	                              );
    	...
    	$.ajax({
    		...
    		success:function(reslut){
    			if(reslut.error_code==1){
    				location.href="${root}/adm/stored.html";
    			}
    		}
    	})
    }
    ...
    function pageChange(pageNo) {
             ...
             var div = $("<div style='size:5px;'></div>").attr("named","hehe").append(function(){
